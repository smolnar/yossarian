module Lastfm::Event
  module Parser
    def self.parse(data)
      ...
      result.artists         = Array.wrap(data[:artists][:artist])
      ...
      result.starts_at       = Time.parse(data[:startDate]) if data[:startDate]
      result.ends_at         = Time.parse(data[:endDate]) if data[:endDate]
